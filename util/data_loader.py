...
num_classes_dict = {'CIFAR-10': 10, 'imagenet': 1000,}
...
def get_loader_out(args, dataset=('tim', 'noise'), config_type='default', split=('train', 'val')):
    ...
    if 'val' in split:
        ...
        if val_dataset == 'dtd':
            ...
            val_ood_loader = torch.utils.data.DataLoader(torchvision.datasets.ImageFolder(root="data/dtd/images/", transform=transform),
                                                       batch_size=batch_size, shuffle=True, num_workers=2)

        elif val_dataset == 'SVHN':
            val_ood_loader = torch.utils.data.DataLoader(torchvision.datasets.SVHN(root='data', split = 'test', download=True, transform=transform_test),
                batch_size=batch_size, shuffle=True, num_workers=2)

        elif val_dataset == 'places365':
            val_ood_loader = torch.utils.data.DataLoader(torchvision.datasets.ImageFolder(root="data/places365/", transform=transform_test),
                                                       batch_size=batch_size, shuffle=True, num_workers=2)
            
        elif val_dataset == 'inat':
            val_ood_loader = torch.utils.data.DataLoader(torchvision.datasets.ImageFolder(root="data/iNaturalist/", transform=config.transform_test_largescale),
                                                       batch_size=batch_size, shuffle=True, num_workers=2)
        elif val_dataset == 'sun50':
            val_ood_loader = torch.utils.data.DataLoader(torchvision.datasets.ImageFolder(root="data/SUN/", transform=config.transform_test_largescale),
                                                       batch_size=batch_size, shuffle=True, num_workers=2)
        elif val_dataset == 'places50': 
            val_ood_loader = torch.utils.data.DataLoader(torchvision.datasets.ImageFolder(root="data/Places/", transform=config.transform_test_largescale),
                                                       batch_size=batch_size, shuffle=True, num_workers=2)
        else:
            val_ood_loader = torch.utils.data.DataLoader(torchvision.datasets.ImageFolder("data/{}".format(val_dataset),
