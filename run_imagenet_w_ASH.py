...
train_mean = ash_s(torch.mean(feat_log, dim= 0).to(device))
...
#pdb.set_trace()
ash_score_log_val = torch.matmul(ash_s(feat_log_val, 90), w.T) + b 
values, nn_idx = ash_score_log_val.max(1)
logits_sub = torch.abs(score_log_val - values.repeat(1000, 1).T)
#pdb.set_trace()
score_in = torch.sum(logits_sub/denominator_matrix[nn_idx], axis=1)/torch.norm(ash_s(feat_log_val, 90) - train_mean , dim = 1)
...
for ood_dataset, (feat_log, score_log) in ood_feat_score_log.items():
    ...
    ash_score_log = torch.matmul(ash_s(feat_log, 90), w.T) + b 
    values, nn_idx = ash_score_log.max(1)
    logits_sub = torch.abs(score_log - values.repeat(1000, 1).T)
    scores_out_test = torch.sum(logits_sub/denominator_matrix[nn_idx], axis=1)/torch.norm(ash_s(feat_log, 90) - train_mean , dim = 1)
...
metrics.print_all_results(all_results, args.out_datasets, 'fDBD w/ ASH')
print()